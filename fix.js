const fs = require('fs');
let content = fs.readFileSync('C:/Users/1306s/.gemini/antigravity/scratch/green-path-tracker/app.js', 'utf8');

const marker = "} else if (action === 'toggle-theme') {";
const index = content.indexOf(marker);

if (index !== -1) {
    const start = content.substring(0, index + marker.length);
    const remainder = `
        toggleDarkMode();
...
function quickLogHabit(co2, leaves, name) {
    ...
    const endMarker = "state.habits.unshift({";
    const endIndex = content.indexOf(endMarker, index);
    
    if (endIndex !== -1) {
        const newContent = start + remainder + "\n    " + content.substring(endIndex);
        fs.writeFileSync('C:/Users/1306s/.gemini/antigravity/scratch/green-path-tracker/app.js', newContent, 'utf8');
        console.log('Fixed');
    } else {
